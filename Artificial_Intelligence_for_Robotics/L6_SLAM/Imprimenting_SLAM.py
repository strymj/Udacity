...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    XiX = XiX.expand(num_landmarks+1, 1, [0], [0])
    XiY = XiY.expand(num_landmarks+1, 1, [0], [0])

    
    # write motions and measurements relation
    for i in range(len(data)):
        ...
        # expand matrix
        matrix_size = num_landmarks+1 + i + 1
        matrix_position=[]
        for j in range(matrix_size):
            matrix_position.append(j)
        matrix_position.remove(i+1)

        OmegaX = OmegaX.expand(matrix_size, matrix_size, matrix_position, matrix_position)
        OmegaY = OmegaY.expand(matrix_size, matrix_size, matrix_position, matrix_position)
        XiX = XiX.expand(matrix_size, 1, matrix_position, [0])
        XiY = XiY.expand(matrix_size, 1, matrix_position, [0])


        # write measurements relation
        for k in range(len(measurement)):

            robo_col = i + 1
            land_col = robo_col + measurement[k][0] + 1

            OmegaX.value[robo_col][robo_col] +=  1 / measurement_noise
            OmegaX.value[land_col][land_col] +=  1 / measurement_noise
            OmegaX.value[land_col][robo_col] += -1 / measurement_noise
            OmegaX.value[robo_col][land_col] += -1 / measurement_noise
            XiX.value[robo_col][0]           += -measurement[k][1] / measurement_noise
            XiX.value[land_col][0]           +=  measurement[k][1] / measurement_noise

            OmegaY.value[robo_col][robo_col] +=  1 / measurement_noise
            OmegaY.value[land_col][land_col] +=  1 / measurement_noise
            OmegaY.value[land_col][robo_col] += -1 / measurement_noise
            OmegaY.value[robo_col][land_col] += -1 / measurement_noise
            XiY.value[robo_col][0]           += -measurement[k][2] / measurement_noise
            XiY.value[land_col][0]           +=  measurement[k][2] / measurement_noise


        # write motion relation
        OmegaX.value[robo_col-1][robo_col-1] +=  1 / motion_noise
        OmegaX.value[robo_col][robo_col]     +=  1 / motion_noise
        OmegaX.value[robo_col][robo_col-1]   += -1 / motion_noise
        OmegaX.value[robo_col-1][robo_col]   += -1 / motion_noise
        XiX.value[robo_col-1][0]             += -motion[0] / motion_noise
        XiX.value[robo_col][0]               +=  motion[0] / motion_noise

        OmegaY.value[robo_col-1][robo_col-1] +=  1 / motion_noise
        OmegaY.value[robo_col][robo_col]     +=  1 / motion_noise
        OmegaY.value[robo_col][robo_col-1]   += -1 / motion_noise
        OmegaY.value[robo_col-1][robo_col]   += -1 / motion_noise
        XiY.value[robo_col-1][0]             += -motion[1] / motion_noise
        XiY.value[robo_col][0]               +=  motion[1] / motion_noise
    ...
    # integrate muX muY
    mu = matrix([[0]])
    mu = mu.expand(matrix_size*2, 1, [0], [0])
    for l in range(len(muX.value)):
        mu.value[l*2] = muX.value[l]
