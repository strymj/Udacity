...
def plan(warehouse, dropzone, todo):
    ...
    for i in range(len(todo)):
        expand = [[-1 for rows in range(len(warehouse[0]))] for cols in range(len(warehouse)) ]
        expand[dropzone[0]][dropzone[1]] = cost
        lists = []
        lists.append([dropzone[0], dropzone[1], cost])
        ...
        while get!=True :
            ...
            for j in range(len(lists)):
                if minnum == -1 or lists[j][2]<lists[minnum][2] :
                    minnum = j
            now = lists[minnum]
            lists.pop(minnum)
            
            for j in range(len(movelists)):
                nlist = [0,0,0] 
                nlist[0] = now[0] + movelists[j][0]
                nlist[1] = now[1] + movelists[j][1]
                nlist[2] = now[2] + movelists[j][2]
                if 0<=nlist[0] and nlist[0]<len(warehouse) and 0<=nlist[1] and nlist[1]<len(warehouse[0]) :
                    if expand[nlist[0]][nlist[1]]==-1 :
                        if warehouse[nlist[0]][nlist[1]] == todo[i] :
                            get = True
                            cost = nlist[2]
                            warehouse[nlist[0]][nlist[1]] = 0
                            print "<get>\ncost :",cost
                            break
                        if warehouse[nlist[0]][nlist[1]] == 0:
                            lists.append(nlist)
