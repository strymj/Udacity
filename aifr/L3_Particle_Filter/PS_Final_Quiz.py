...
class robot:
    ...
    def move(self, motion):
        ...
        steering = motion[1] / self.length * tan(motion[0])
        distance = motion[1]
        
        steering2 = random.gauss(steering,steering_noise)
        distance2 = random.gauss(distance,distance_noise)
        
        if fabs(steering2) < threshold:
            res.x = self.x + distance2 * cos(self.orientation)
            res.y = self.y + distance2 * sin(self.orientation)
        else:
            R = distance2 / steering2
            cx = self.x - R * sin(self.orientation)
            cy = self.y + R * cos(self.orientation)
            res.x = cx + R * sin(self.orientation + steering2)
            res.y = cy - R * cos(self.orientation + steering2)
        res.orientation = (self.orientation + steering2) % (2*pi)
    ...
    def sense(self,flag):
        ...
        for i in range(len(landmarks)):
            ...
            if flag != 0:
                bearing = (random.gauss(bearing,bearing_noise))
